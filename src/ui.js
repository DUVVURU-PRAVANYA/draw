export class UIManager {
  constructor(drawingEngine, callbacks) {
    ...
    this.hudTrackVal    = document.getElementById('hud-track-val');
    ...
    this.panelHeader   = document.getElementById('panel-header-mobile') || document.querySelector('.panel-header-mobile');
  ...
  _bindEvents() {
    ...
    this.colorBtns.forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        this.showToast(`🎨 Palette: ${btn.dataset.color}`);
      });
    });

    this.sizeInput?.addEventListener('input', e => {
      if (this.sizeVal) this.sizeVal.textContent = e.target.value;
      this.drawingEngine.setSize(+e.target.value);
    });

    this.glowInput?.addEventListener('input', e => {
      if (this.glowVal) this.glowVal.textContent = e.target.value;
      this.drawingEngine.setGlow(+e.target.value);
    });

    this.eraserInput?.addEventListener('input', e => {
      if (this.eraserVal) this.eraserVal.textContent = e.target.value;
    ...
    this.downloadBtn?.addEventListener('click', () => {
      ...
      this.showToast('⬇️ Saved Artwork as PNG');
    });
  ...
  updateGestureLabel(gesture, handDetected) {
    ...
    if (this.hudTrackVal) {
      this.hudTrackVal.textContent = handDetected ? (gesture === 'DRAW' ? 'DRAWING' : 'ACTIVE') : 'STANDBY';
      this.hudTrackVal.style.color = handDetected ? '#00FFAA' : '#8A99AD';
    }
    ...
    if (this.handStatus) {
      if (handDetected && gesture === 'DRAW') {
        ...
        this.handStatus.textContent = 'Color Cycle 🎨';
        this.handStatus.className   = 'hand-status peace';
      } else if (handDetected) {
        this.handStatus.textContent = 'Hand Active ✅';
